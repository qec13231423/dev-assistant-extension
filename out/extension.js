...
function activate(context) {
    const disposable = vscode.commands.registerCommand('dev-assistant.openPanel', () => {
        const panel = vscode.window.createWebviewPanel('devAssistant', 'Dev Assistant', vscode.ViewColumn.One, { enableScripts: true });
        ...
        panel.webview.onDidReceiveMessage(message => {
            switch (message.command) {
                case 'generateTests':
                    ...
                    break;
                case 'fixSnyk':
                    vscode.window.showInformationMessage('🔧 Fixing Snyk issues...');
                    break;
            }
        });
    });
